...
?>
    ...
    <div class="card mb-5">
        <div class="card-body">
            <p>Nombre del Jefe: <?= sanear_string($dataB[6] . " " . $dataB[7] . " " . $dataB[8] . " " . $dataB[9]); ?></p>
            <p>Área: <?= sanear_string($dataB[0]); ?></p>
            <p>Cargo: <?= sanear_string($dataB[1]); ?></p>
        </div>
    </div>
...
?>
    ...
    <div class="card mb-5">
        <div class="card-body">
            <p>Nombre Empleado: <?= sanear_string($dataE[0] . " " . $dataE[1] . " " . $dataE[2] . " " . $dataE[3] . " " . $dataE[4]); ?></p>
            <p>Tipo de Contrato: <?= sanear_string($dataP[3]); ?></p>
            <p>Fecha de Contrato: <?= sanear_string($dataP[6]); ?></p>
            <p>Salario: $<?= sanear_string($dataP[5]); ?></p>
        </div>
    </div>
<?php
} else {
...
if ($positionHistory->rowCount() > 0) {
    while ($data = $positionHistory->fetch()) {
        ...
        <div class="card-body">
              <p>Nombre Empleado: ' . sanear_string($dataE[0] . ' ' . $dataE[1] . ' ' . $dataE[2] . ' ' . $dataE[3]) . '</p>
              <p>Tipo de Contrato: ' . sanear_string($data[3]) . '</p>
              <p>Fecha de Contrato: ' . sanear_string($data[6]) . '</p>';
        if ($data[7] == "0000-00-00") {
            echo '<p>Fecha de finalización: Empleado Activo</p>';
        } else {
            echo '<p>Fecha de finalización: ' . sanear_string($data[7]) . '</p>';
        }
        echo '<p>Salario: $' . sanear_string($data[5]) . '</p>
        </div>
      </div>';
    }
} else {
